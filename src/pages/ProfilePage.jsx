...
const ProfilePage = () => {
  ...
  const { myProfile, profileUser, status, error } = useSelector(
    (state) => state.profile
  );
  ...
  const fetchFollowers = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/users/${userToShow._id}/followers`,
        {
          headers: {
  ...
  const fetchFollowing = async () => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/users/${userToShow._id}/following`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
            "Content-Type": "application/json",
              ...
              <div className="flex-1 justify-center w-full">
                {isEditing ? (
                  ...
                  <h1 className="text-xl sm:text-2xl font-bold text-white">
                    {formatName(formData.name || userToShow?.name || "User")}
                  </h1>
                )}
              </div>
            </div>
        ...
        <section className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <p className="text-sm text-gray-500 mt-2">
            ...
            {userToShow?.createdAt
              ? new Date(userToShow.createdAt).toLocaleDateString()
              : "Unknown"}
          </p>
